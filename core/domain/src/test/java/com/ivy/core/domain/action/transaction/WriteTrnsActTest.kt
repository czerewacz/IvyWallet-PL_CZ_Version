...
class WriteTrnsActTest {
    ...
    // Save a new transaction and make sure that it is saved in the database
    ...
    fun `Test create new transaction with expense`() = runBlocking<Unit> {
        ...
        val cachedTransaction = transactionDaoFake.transactionEntities.find {
            it.id == transactionId.toString()
        }
        val cachedTag = transactionDaoFake.trnTagEntities.find { it.tagId == tag.id }
